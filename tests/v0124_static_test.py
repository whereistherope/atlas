...
assert "APP_VERSION='0.12.4'" in db and 'DATA_VERSION=8' in db
...
assert index.index('./js/db.js')<index.index('./js/auth.js')<index.index('@supabase/supabase-js@2.111.0')<index.index('./js/cloud-config.js')<index.index('./js/cloud.js')<index.index('./js/app.js')<index.index('./js/relay.js')<index.index('./js/calendar.js')<index.index('./js/map.js')<index.index('./js/ui.js')<index.index('./js/widgets.js')<index.index('./js/bootstrap.js')
assert './js/cloud-config.js' in sw and './js/cloud.js' in sw and 'cdn.jsdelivr.net' not in sw
assert "(state.settings.activeProfile||'me')!=='me'" in widgets and 'LOCAL ONLY' in widgets and 'status.verified' in widgets
assert "eq('created_by',user.id)" in cloud and "eq('name','Atlas')" in cloud and '.limit(1)' not in cloud
assert "eq('profile_key','me')" in cloud and "eq('kind','person')" in cloud and "eq('owner_user_id',user.id)" in cloud
assert not any(x in cloud for x in ('.insert(','.update(','.delete(',"from('atlas_records')"))
