...
assert(bootstrap.includes("const BUILD='0169r17'"),'bootstrap build must advance with native refinement');
assert(bootstrap.includes("loadStyle('./styles/lock-terrain.css')"),'lock identity stylesheet must load');
assert(bootstrap.includes("loadScript('./js/lock-terrain.js','Atlas lock identity v0.16.9-r17')"),'lock identity runtime must load');
assert(sw.includes("atlas-shell-0.16.9-r17"),'service worker cache must advance with native refinement');
...
console.log('Atlas artwork-free lock identity: PASS');
