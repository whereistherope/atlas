...
assert(bootstrap.includes("const BUILD='0169r20'"),'bootstrap build must advance with widget visibility hotfix');
assert(bootstrap.includes("loadStyle('./styles/lock-terrain.css')"),'lock identity stylesheet must load');
assert(bootstrap.includes("loadScript('./js/lock-terrain.js','Atlas lock identity v0.16.9-r17')"),'lock identity runtime must remain unchanged through r20');
assert(sw.includes("atlas-shell-0.16.9-r20"),'service worker cache must advance with widget visibility hotfix');
...
console.log('Atlas artwork-free lock identity preserved through r20: PASS');
