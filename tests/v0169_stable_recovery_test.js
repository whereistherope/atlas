...
const material25=fs.readFileSync('styles/r25-note-field-polish.css','utf8');
...
assert(bootstrap.includes("const BUILD='0169r25'"),'r25 build missing');
...
assert(bootstrap.includes("loadStyle('./styles/r25-note-field-polish.css')"),'r25 note field polish must load last');
...
assert(material22.includes('--atlas-float-glass:rgba(7,11,15,.46)'),'r22 glass base must remain beneath r25');
assert(material25.includes('--atlas-float-glass:rgba(7,11,15,.51)'),'r25 night window material must gain one further opacity step');
assert(material25.includes('--atlas-editor-field-fill:#11171c'),'r25 night editor field token missing');
assert(material25.includes('.atlas-vnote-fields input'),'visual note inputs must share explicit field fill');
assert(material25.includes('.atlas-vnote-fields select'),'visual note selects must share explicit field fill');
assert(material25.includes('.atlas-vnote-body'),'visual note body must share explicit field fill');
assert(material25.includes('background:var(--atlas-editor-field-fill)!important'),'editor controls must use exact shared fill token');
...
assert(sw.includes("atlas-shell-0.16.9-r25"),'r25 cache version missing');
assert(sw.includes("'./styles/r25-note-field-polish.css'"),'r25 visual polish must be cached');
...
console.log('Atlas v0.16.9-r25 note/editor material + Shared Atlas compatibility: PASS');
