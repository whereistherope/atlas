...
const legacyUpcomingScroll=read('house/legacy-upcoming-scroll.js');
const legacyUpcomingScrollCss=read('house/legacy-upcoming-scroll.css');
...
for(const asset of ['./house/','./house/index.html','./house/legacy.html','./house/legacy.css','./house/legacy.js','./house/legacy-calendar.css','./house/legacy-calendar.js','./house/legacy-upcoming-scroll.js','./house/legacy-upcoming-scroll.css','./house/legacy-upcoming-edit.js','./house/legacy-calendar-link-colours.js','./house/legacy-calendar-link-ui.js','./house/legacy-calendar-person.js','./house/legacy-calendar-person.css'])assert.ok(sw.includes(asset),`${asset} route shell is not offline-cached`);
...
assert.match(entry,/legacy\?'\.\/legacy\.html\?compat=r66':'\.\.\/\?view=house'/,'modern devices must continue into the shared Atlas House route');
assert.match(entry,/Compatibility view/,'House routing shell must expose a manual legacy fallback link');
assert.match(legacy,/\.\/legacy\.css\?v=r66/,'legacy House CSS missing');
assert.match(legacy,/\.\/legacy\.js\?v=r66/,'legacy House runtime missing');
assert.match(legacy,/\.\/legacy-calendar\.css\?v=r66/,'legacy House calendar CSS missing');
assert.match(legacy,/\.\/legacy-calendar\.js\?v=r66/,'legacy House calendar writer missing');
assert.match(legacy,/\.\/legacy-upcoming-scroll\.css\?v=r66/,'legacy House upcoming scroll styling missing');
assert.match(legacy,/\.\/legacy-upcoming-scroll\.js\?v=r66/,'legacy House upcoming scroll runtime missing');
assert.match(legacy,/\.\/legacy-upcoming-edit\.js\?v=r66/,'legacy House upcoming editor missing');
assert.match(legacy,/\.\/legacy-calendar-link-colours\.js\?v=r66/,'legacy House calendar colour/link writer missing');
assert.match(legacy,/\.\/legacy-calendar-link-ui\.js\?v=r66/,'legacy House calendar profile-link UI missing');
assert.match(legacy,/\.\/legacy-calendar-person\.js\?v=r66/,'legacy House calendar Who runtime missing');
assert.match(legacy,/\.\/legacy-calendar-person\.css\?v=r66/,'legacy House calendar Who styling missing');
...
assert.doesNotMatch(legacyUpcomingScroll,/\?\.|\?\?|=>|\basync\b|\bawait\b|`/,'legacy House upcoming scroll runtime must stay parseable by iOS 12 Safari');
...
assert.doesNotMatch(legacyUpcomingScrollCss,/color-mix\(|100dvh|100svh|100lvh/,'legacy House upcoming scroll CSS must avoid modern-only presentation features');
...
assert.match(legacyUpcomingScroll,/function upcoming\(\)/,'legacy House must materialise the full 30-day Upcoming set');
assert.doesNotMatch(legacyUpcomingScroll,/\.slice\(0,[1-9]/,'legacy House Upcoming must not truncate the 30-day list');
assert.match(legacyUpcomingScroll,/data-house-calendar-id/,'full legacy House Upcoming rows must stay editable');
assert.match(legacyUpcomingScroll,/atlas-house-scroll-rail/,'legacy House must render an Atlas-owned scroll indicator');
assert.match(legacyUpcomingScroll,/list\.onscroll=updateRail/,'legacy House scroll indicator must follow touch scrolling');
assert.match(legacyUpcomingScrollCss,/overflow-y:scroll/,'legacy House Upcoming must be vertically scrollable');
assert.match(legacyUpcomingScrollCss,/background:var\(--accent\)/,'legacy House scroll thumb must use Atlas accent colour');
...
assert.match(context,/root\.AtlasHouse\?\.isActive\?\.\(\)\?allEvents:allEvents\.slice\(0,8\)/,'only House must expand Upcoming beyond the normal eight-item preview');
assert.match(context,/`\$\{allEvents\.length\} \/ 30D`/,'House Upcoming meta must count the full 30-day set');
...
assert.match(css,/\.house-atlas-board \.widget-body\{overflow:hidden!important;max-height:none!important\}/,'House widgets must remain bounded by default');
assert.match(css,/\.house-upcoming \.widget-list\{[^}]*overflow-y:auto!important/,'House Upcoming must be the bounded scroll exception');
assert.match(css,/scrollbar-color:var\(--accent\) transparent/,'House Upcoming scrollbar must use Atlas colours');
assert.match(css,/::-webkit-scrollbar-thumb\{background:var\(--accent\)/,'Safari House scrollbar must use the Atlas accent thumb');
...
console.log('atlas house shared widgets + r66 upcoming scroll contract ok');
