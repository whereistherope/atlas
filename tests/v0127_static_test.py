...
assert "'./js/relay-transport.js'" in sw and 'atlas-shell-0.12.8-r1' in sw
...
assert 'AtlasRelay.ingest(payload)' in transport and 'setTimeout' in transport
assert 'fetch_atlas_relay' in transport and 'ack_atlas_relay' in transport and 'AtlasRelayTransport' in transport
assert 'relay-cloud-check' in (root/'js/widgets.js').read_text() and 'CHECK NOW' in (root/'js/widgets.js').read_text()
