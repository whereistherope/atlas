...
for(const token of [
  ...
  "timeParts('UTC')",
  'topographyText(cols=118,rows=76)',
  'terrainMarkup()',
  "root.AtlasLockTopography=Object.freeze({version:'0.16.7-r3'",
]) assert(runtime.includes(token),`missing lock runtime contract: ${token}`);

assert(!runtime.includes('profileLabel('),'lock screen must not render a profile label');
assert(!runtime.includes('data-lock-profile'),'lock screen must not inject profile chrome');
...
assert(bootstrap.includes("const BUILD='0167r3'"),'bootstrap build must be v0.16.7-r3');
assert(bootstrap.includes("loadStyle('./styles/lock-topography.css')"),'lock stylesheet must load');
assert(bootstrap.includes("loadScript('./js/lock-topography.js','Atlas lock topography v0.16.7-r3')"),'lock runtime must load');
assert(sw.includes("atlas-shell-0.16.7-r3"),'service worker cache must be bumped');
...
console.log('Atlas v0.16.7-r3 full-field lock topography presentation: PASS');
