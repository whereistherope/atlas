...
assert(!layout.includes('ROOT_RX'),'root orbit must be circular, not elliptical');
assert(!layout.includes('ROOT_RY'),'root orbit must be circular, not elliptical');
...
assert(layout.includes("svg.querySelectorAll('.edge.cross')"),'only associative cross-links should be centre-routed');
assert(layout.includes("${hub.x.toFixed(2)} ${hub.y.toFixed(2)}"),'cross-link path must explicitly pass through the central hub');
...
assert(bootstrap.includes("const BUILD='0158r1'"),'bootstrap build not bumped');
assert(sw.includes("atlas-shell-0.15.8-r1"),'service worker cache not bumped');
assert(layout.includes("version:'0.15.8-r1'"),'network layout export version missing');

console.log('Atlas v0.15.8 sun-centred network grammar: PASS');
