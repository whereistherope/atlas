...
async function checkPage(browser,url,label,verify){
  ...
  await page.waitForFunction(()=>document.documentElement.classList.contains('atlas-ready'),null,{timeout:15000});
  await page.waitForFunction(()=>!document.getElementById('atlasSimpleLoader'),null,{timeout:5000});
...
(async()=>{
  ...
  try{
    ...
    await checkPage(browser,base+'/?view=house','Atlas House',async page=>{
      await page.waitForSelector('#atlasHouseBoard',{state:'attached',timeout:5000});
...
})().catch(error=>{console.error(error&&error.stack||error);server.close(()=>process.exit(1));});
