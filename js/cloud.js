...
(function(){
  ...
  const FORBIDDEN_BACKUP_KEYS=new Set(['password','pin','pinhash','hash','lock','atlaslock','lockconfig','salt','verifier','throttle','attempts','lockeduntil','recovery','recoverycode','recoverycredentials','secret','servicerole','token','accesstoken','refreshtoken','session','supabase','auth','entangledid','sourceeventid','sourcearealabel']);
  ...
  function validBackupPayload(payload){
    ...
    if(payload.schema!=='atlas_me_backup_snapshot'||payload.version!==1||payload.profileKey!=='me'||!Number.isInteger(payload.dataVersion)||payload.dataVersion<1)return false;
    ...
    return validBackupValue(payload);
  }
  ...
  window.AtlasCloud=Object.freeze({init,getStatus:snapshot,getSession:refreshSession,signIn,signOut,testAccess,invalidateVerification,meBackupExists,appendMeBackupSnapshot});
})();
