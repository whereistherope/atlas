...
const WIDGET_LOAD={scratch:2.8,todo:1.8,upcoming:2.4,calendar:3.1,active:2.5,milestones:2.3,projects:2.3,notes:2.2,log:2.2};
function adaptiveWidgetZones(){
  // Preserve the user's dock choices across aspect ratios. The bottom zone is a
  // real centre lane beneath the map on wide screens and returns to normal flow
  // on narrow screens. No widgets are silently reassigned to another dock.
...
function networkPanel(scope=null){
  ...
  return `<section class="network-stage ${hero?'home-network':'area-network'}">${scope?`<div class="network-head"><h2>${esc(areaById(scope)?.name||'Area')} Network</h2></div>`:''}<div class="map-wrap ${predict?'prediction-mode':''}"><div class="map-topline"><div class="map-meta"><span>${mode==='nodes'?'MAP':mode==='list'?'LIST':'PREDICT'}</span><span>${esc(activeProfile().name.toUpperCase())}</span><span>${predict?`${metaCount} PROJECTED`:`${count} NODES`}</span><span id="mapInspect"></span></div><div class="map-view-toggle"><button type="button" data-map-view="nodes" class="${mode==='nodes'?'active':''}">Nodes</button><button type="button" data-map-view="list" class="${mode==='list'?'active':''}">List</button><button type="button" data-map-view="predict" class="${mode==='predict'?'active':''}">Predict</button></div></div>${mode==='list'?`<div class="branch-view">${branchTreeHtml(scope)}</div>`:`<svg id="network" viewBox="0 0 1200 680" role="img" aria-label="${predict?'Atlas procedural prediction map':'Atlas relationship map'}"></svg>${predict?'<aside id="predictInspector" class="prediction-inspector" aria-live="polite"></aside>':''}<div class="map-controls"><div class="map-hud"><div class="zoom-controls"><button type="button" data-map-zoom="out">−</button><button type="button" class="zoom-reset" data-map-zoom="reset"><span id="zoomValue">FIT</span></button><button type="button" data-map-zoom="in">+</button></div>${predict?`<div class="zoom-controls"><button type="button" class="zoom-reset" data-predict-regenerate>Regenerate</button></div><span class="predict-note">CONTEXTUAL PROJECTION · AI SLOTS MARKED · NOT STORED</span>`:`<div class="zoom-controls"><button type="button" class="zoom-reset" data-map-layout="organise">Reform</button></div><div class="depth">Depth <input id="depthRange" type="range" min="2" max="5" step="1" value="${state.settings.mapDepth}"/><strong id="depthValue">${state.settings.mapDepth}</strong></div><div class="depth map-opacity">Type <input id="labelOpacityRange" type="range" min="0" max="100" step="1" value="${Math.round((state.settings.mapLabelOpacity??.72)*100)}"/><strong id="labelOpacityValue">${Math.round((state.settings.mapLabelOpacity??.72)*100)}</strong></div><div class="depth map-opacity">Links <input id="edgeOpacityRange" type="range" min="0" max="100" step="1" value="${Math.round((state.settings.mapEdgeOpacity??.32)*100)}"/><strong id="edgeOpacityValue">${Math.round((state.settings.mapEdgeOpacity??.32)*100)}</strong></div>`}</div></div>`}</div></section>`;
}
