...
function adaptiveWidgetZones(){
  const zones={top:widgetZoneItems('top'),left:widgetZoneItems('left'),right:widgetZoneItems('right'),bottom:widgetZoneItems('bottom'),float:widgetZoneItems('float')};
  // Bottom is the user's general intent, not an unusable fixed coordinate. On
  // roomy canvases, pack its widgets into a short side lane first; their saved
  // zone/order remain untouched and narrow layouts return them to normal flow.
  if(window.innerWidth>=1180){
    const load=ids=>ids.reduce((sum,id)=>sum+(WIDGET_LOAD[id]||2),0);let leftLoad=load(zones.left),rightLoad=load(zones.right);
    zones.bottom.slice().forEach(id=>{if(Math.min(leftLoad,rightLoad)>=5.4)return;if(leftLoad<=rightLoad){zones.left.push(id);leftLoad+=WIDGET_LOAD[id]||2}else{zones.right.push(id);rightLoad+=WIDGET_LOAD[id]||2}zones.bottom=zones.bottom.filter(x=>x!==id)});
  }
  return zones;
}
