...
(function(root){
  ...
  function goHome(mode=null){
    state.settings.activeTab='home';
    state.settings.selectedArea='';
    state.settings.subtab='overview';
  ...
  function openArea(id){
    const a=areaById(id);
    if(!a||!profileAllows(a.profile))return root.toast?.('Area unavailable');
    state.settings.activeTab=getTopDomain(a)?.id||a.id;
    state.settings.selectedArea=a.id;
    state.settings.subtab='overview';
    renderTabs();
    renderArea(a.id);
    return save?.();
  ...
  function openProjectAction(id){
    ...
    if(!p||!profileAllows(p.profile))return root.toast?.('Project unavailable');
    return openProject?.(id);
  }

  function openNoteAction(id){
    const n=state.notes.find(x=>x.id===id);
    if(!n||!profileAllows(n.profile))return root.toast?.('Note unavailable');
    return root.AtlasMarkdown?.openNote?.(id);
  }

  function openEventAction(id){
    const e=state.calendar.find(x=>x.id===id);
    if(!e||!profileAllows(e.profile))return root.toast?.('Event unavailable');
    return openCalendarEvent?.(id);
  }

  function closeLauncher(){
    try{root.AtlasCaptureFramework?.close?.()}catch(_){document.getElementById('atlasCaptureLauncher')?.classList.remove('open')}
  }
  ...
  function projectOptions(areaId=''){
    ...
    return {
      suggested,
      html:`<option value="">Unassigned</option>`+all.map(p=>`<option value="${h(p.id)}" ${p.id===suggested?'selected':''}>${h(p.code||'PROJECT')} · ${h(p.title)}</option>`).join('')
    };
  }

  function ensureCreateOverlay(){
    let overlay=document.getElementById('atlasDirectCreate');
    if(overlay)return overlay;
    overlay=document.createElement('div');
    overlay.id='atlasDirectCreate';
    overlay.className='overlay atlas-direct-create';
  ...
  function openTaskCreate(areaId=''){
    closeLauncher();
    createKind='task';
    createContext=areaId||currentContextAreaId();
    const overlay=ensureCreateOverlay();
    const area=contextArea(createContext);
    const projects=projectOptions(createContext);
    overlay.querySelector('#atlasDirectCreateCode').textContent='QUICK ACTION';
    overlay.querySelector('#atlasDirectCreateTitle').textContent='New Task';
    ...
    const areaSelect=overlay.querySelector('#atlasDirectArea');
    if(areaSelect)areaSelect.value=createContext||'';
    overlay.classList.add('open');
    setTimeout(()=>overlay.querySelector('#atlasDirectTitle')?.focus(),30);
  }

  function topicParents(){
    return profileAreas().filter(a=>Number(a.level)<4&&spaceAllows(a.space)).sort((a,b)=>Number(a.level)-Number(b.level)||a.name.localeCompare(b.name));
  }

  function preferredTopicParent(areaId=''){
    const ctx=contextArea(areaId);
    if(ctx&&Number(ctx.level)<4)return ctx.id;
    if(ctx&&Number(ctx.level)>=4){
      const parent=areaById(ctx.parentId);
      if(parent&&Number(parent.level)<4)return parent.id;
    }
  ...
  function openStructureCreate(kind,areaId=''){
    ...
    createContext=areaId||currentContextAreaId();
    const overlay=ensureCreateOverlay();
    const ctx=contextArea(createContext);
    const form=overlay.querySelector('#atlasDirectCreateForm');
    overlay.querySelector('#atlasDirectCreateCode').textContent='STRUCTURE';
    overlay.querySelector('#atlasDirectCreateTitle').textContent=kind==='area'?'New Area':'New Topic';
    ...
    if(kind==='area'){
      ...
      const parents=topicParents();
      const selected=preferredTopicParent(createContext);
  ...
  function closeDirectCreate(){
    const overlay=document.getElementById('atlasDirectCreate');
  ...
  async function saveTask(){
    const title=(document.getElementById('atlasDirectTitle')?.value||'').trim();
    if(!title)return root.toast?.('Add a task first');
    const projectId=document.getElementById('atlasDirectProject')?.value||'';
    const areaId=document.getElementById('atlasDirectArea')?.value||'';

    if(projectId){
      const p=projectById(projectId);
      if(!p)return root.toast?.('Project unavailable');
      p.tasks=p.tasks||[];
      p.tasks.push({id:uid('t'),title,done:false});
      log?.(`Task added to ${p.title}: ${title}.`);
      await save();
      renderAll();
      closeDirectCreate();
      root.toast?.(`Task added to ${p.title}`);
      return;
    }

    const area=areaId?areaById(areaId):null;
    const profile=state.settings.activeProfile||'me';
    ...
    log?.(`Unassigned task captured: ${title}.`);
    await save();
    renderAll();
    closeDirectCreate();
    root.toast?.('Task created');
  }

  async function saveStructure(){
    const kind=createKind;
    const label=kind==='area'?'Area':'Topic';
    const name=(document.getElementById('atlasDirectTitle')?.value||'').trim();
    if(!name)return root.toast?.(`Add a ${kind==='area'?'name':'topic name'} first`);

    const profile=state.settings.activeProfile||'me';
    let level,parentId,space;
    if(kind==='area'){
      level=2;
      parentId='atlas';
      space=document.getElementById('atlasDirectSpace')?.value||'personal';
    }else{
      parentId=document.getElementById('atlasDirectParent')?.value||'';
      const parent=areaById(parentId);
      if(!parent)return root.toast?.('Choose a Topic parent');
      level=Math.min(4,Number(parent.level||2)+1);
      space=parent.space||'personal';
    }
    ...
    organiseSphericalLayout(state,profile);
    log?.(`${label} created: ${name}.`);
    await save();
    closeDirectCreate();
    renderAll();
    openArea(item.id);
    root.toast?.(`${label} created`);
  }

  async function submitDirectCreate(event){
    event.preventDefault();
    if(createKind==='task')return saveTask();
    if(createKind==='area'||createKind==='topic')return saveStructure();
  }
  ...
  function installAlignedLauncher(){
    ...
    const choices=[
      ...
      ['meeting','Meeting','Meeting notes with rich text'],
      ['idea','Idea','Capture an idea'],
      ['reference','Reference','Save reference material'],
      ['project','Project','Create a structured project'],
      ['task','Task','Create an action with explicit Project choice'],
      ['daily','Daily','Add a daily entry'],
      ['area','Area','Create a top-level Area directly'],
      ['topic','Topic','Create a Topic directly']
    ];
  ...
  function updateLauncherContext(){
    const el=document.getElementById('atlasCaptureContext');
    if(!el)return;
  ...
  function installCaptureWrappers(){
    ...
    if(baseRich?.open){
      const baseOpen=baseRich.open.bind(baseRich);
      const baseLegacy=baseRich.legacy?.bind(baseRich);
    ...
    if(typeof openCapture==='function'){
      ...
      openCapture=function(type='note',areaId=''){
        if(type==='task')return openTaskCreate(areaId||currentContextAreaId());
        return baseOpenCapture(type,areaId||currentContextAreaId());
      };
    }
  ...
  installAlignedLauncher();
  installCaptureWrappers();
  installActionableWidgets();
  ...
  document.addEventListener('pointerdown',event=>{
    if(event.target.closest?.('#captureBtn'))updateLauncherContext();
  },true);

  document.addEventListener('click',event=>{
    const choice=event.target.closest?.('[data-atlas-aligned-capture]');
    if(choice){event.preventDefault();alignedCapture(choice.dataset.atlasAlignedCapture);return}
    ...
    const p=event.target.closest?.('[data-atlas-open-project]');
    if(p){event.preventDefault();openProjectAction(p.dataset.atlasOpenProject);return}
    const n=event.target.closest?.('[data-atlas-open-note]');
    if(n){event.preventDefault();openNoteAction(n.dataset.atlasOpenNote);return}
    if(event.target===document.getElementById('atlasDirectCreate'))closeDirectCreate();
  },true);

  document.addEventListener('submit',event=>{
    if(event.target?.id==='atlasDirectCreateForm')submitDirectCreate(event);
  },true);

  root.addEventListener('keydown',event=>{
    if(event.defaultPrevented)return;
    if(event.key==='Escape'&&document.getElementById('atlasDirectCreate')?.classList.contains('open')){
      event.preventDefault();
      closeDirectCreate();
    }
  },true);
