...
(function(root){
  ...
  // Direct siblings share equal angular intervals while keeping a quiet inward
  // wedge toward the parent. This is the tighter radial-fan behaviour approved
  // before v0.15.8 widened small fans and caused neighbouring branches to overlap.
  ...
  // Every generation starts from the same expansion step. A crowded sibling fan
  // may grow just enough to preserve minimum clearance, but all siblings in that
  // fan still share exactly one radius.
  ...
  // Associative links represent the actual endpoint relationship. They are not
  // obstacle-routed and do not converge on a synthetic central waypoint: if the
  // straight relationship naturally crosses the central void, it simply does so.
