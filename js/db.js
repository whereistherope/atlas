...
// Secrets which are not part of Atlas state use distinct records in the existing auth store.
function authStoreGet(key){return new Promise(resolve=>{if(!db)return resolve(null);try{const r=db.transaction(AUTH_STORE,'readonly').objectStore(AUTH_STORE).get(key);r.onsuccess=()=>resolve(r.result||null);r.onerror=()=>resolve(null)}catch(_){resolve(null)}})}
function authStoreSet(key,value){return new Promise(resolve=>{if(!db)return resolve();try{const tx=db.transaction(AUTH_STORE,'readwrite');tx.objectStore(AUTH_STORE).put(value,key);tx.oncomplete=()=>resolve();tx.onerror=()=>resolve()}catch(_){resolve()}})}
function authStoreRemove(key){return new Promise(resolve=>{if(!db)return resolve();try{const tx=db.transaction(AUTH_STORE,'readwrite');tx.objectStore(AUTH_STORE).delete(key);tx.oncomplete=()=>resolve();tx.onerror=()=>resolve()}catch(_){resolve()}})}
