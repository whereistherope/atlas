...
function personLabel(event){var value=event&&event.person&&LABELS[event.person]?event.person:personForProfile(event&&event.profile||'us');return LABELS[value]||'Together'}
...
function syncWhoVisibility(){ensureWhoField();var field=byId('houseCalendarPerson')&&byId('houseCalendarPerson').parentNode,type=byId('houseCalendarType')&&byId('houseCalendarType').value;if(field)field.style.display=type==='travel'?'none':'block'}
...
function saveNewEvent(){
  ...
  ensureSession().then(function(session){return ensureTarget(session).then(function(target){return insertRows(session,target,profile?[source,shared]:[shared])})}).then(function(){byId('houseCalendarOverlay').className='calendar-overlay hidden';showMessage(profile?'Linked to '+profileLabel(profile)+' and Us':'Event added to Us');var refresh=byId('refreshHouse');if(refresh)refresh.click()}).catch(function(error){showMessage(error&&error.message?error.message:'Calendar event was not saved.')}).then(function(){button.disabled=false;button.innerHTML='Save Event'})
}
function saveEditedEvent(){
  ...
  })}).then(function(){byId('houseCalendarOverlay').className='calendar-overlay hidden';showMessage('Event updated');currentHouseId='';var refresh=byId('refreshHouse');if(refresh)refresh.click()}).catch(function(error){showMessage(error&&error.message?error.message:'Calendar event was not saved.')}).then(function(){button.disabled=false;button.innerHTML='Save Changes'})
}
...
function decorateUpcoming(){var list=byId('upcomingList');if(!list)return;var rows=list.querySelectorAll('[data-house-calendar-id]'),i,row,event,body,small;for(i=0;i<rows.length;i++){row=rows[i];event=cachedEvent(row.getAttribute('data-house-calendar-id'));if(!event||event.entryType==='travel')continue;body=row.querySelector('div');if(!body)continue;body.innerHTML='<strong class="house-event-person">'+personLabel(event)+'</strong><span class="house-event-title">'+String(event.title||'Untitled event').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;')+'</span><small>'+String(event.date||'')+(timeMeta(event)?' · '+timeMeta(event):'')+'</small>';small=body.querySelector('small');if(small)small.style.display='block'}}
